...
impl TerminalRenderer {
    ...
    pub fn layout_row(
        ...
        for (col, cell) in cells {
            ...
            // Extract cell styling, handling INVERSE (reverse video)
            let inverse = cell.flags.contains(Flags::INVERSE);
            let (fg_color, bg_color) = if inverse {
                (self.palette.resolve(cell.bg, colors), self.palette.resolve(cell.fg, colors))
            } else {
                (self.palette.resolve(cell.fg, colors), self.palette.resolve(cell.bg, colors))
            };
    ...
    pub fn paint(
        ...
        // Iterate over visible lines, accounting for scroll offset
        for line_idx in 0..num_lines {
            ...
            for (col_idx, cell) in cells {
                ...
                // Get cell colors, handling INVERSE (reverse video)
                let flags = cell.flags;
                let fg_color = if flags.contains(Flags::INVERSE) {
        ...
        // Paint cursor only when visible and viewport is at the bottom
        ...
        if show_cursor {
            ...
            if cursor_line >= 0 && (cursor_line as usize) < num_lines {
                ...
                let cursor_y = origin.y + self.cell_height * (cursor_line as f32);
                ...
                // Re-render character under cursor with inverted colors so it's
                // visible on top of the block (text renders above quads in GPUI)
                let col = cursor_point.column;
                let line = AlacPoint::new(cursor_point.line, col);
                let cell = &grid[line];
                let ch = cell.c;
                if ch != ' ' && ch != '\0' {
                    let font = Font {
                    ...
                    let text_y = cursor_y + vertical_offset;
                    let _ = shaped_line.paint(
                        Point { x: cursor_x, y: text_y },
                        self.cell_height,
                        window,
                        _cx,
                    );
